...
describe('linq', () => {
  ...
  it('should yield repeatable results', () => {
    ...
    testRepeatableResolution('xyzxyzxyz', linqInstance.transform(notNull), joinStrings());
  });
  ...
  describe('.filter(...filters)', () => {
    it('should filter results', () => {
      testRepeatableResolution(
        ['y'],
        linqInstance.transform(notNull).filter(
          where((e) => e === 'y'),
          distinct
        )
      );
...
describe('linqExtended', () => {
  ...
  describe('.where(predicate)', () => {
    it('should filter results', () => {
      ...
      testRepeatableResolution(
        'xyxyxy',
        linqInstance.transform(notNull).where((e) => e !== 'z'),
        joinStrings()
      );
    });
  });
  ...
  describe('.toArray()', () => {
    it('should filter results', () => {
      // noinspection SpellCheckingInspection
      testRepeatableDelegate('xyzxyzxyz', () => linqInstance.transform(notNull).toArray().join(''));
    });
  });
  ...
  describe('.select()', () => {
    it('should filter results', () => {
      // noinspection SpellCheckingInspection
      testRepeatableDelegate('abcdef', () =>
        linqExtended(testItems)
          .select((o) => o.c)
          .toArray()
          .join('')
      );
  ...
  describe('.groupBy(selector)', () => {
    it('should be able to cherry pick item', () => {
      ...
      testRepeatableResolution('y', grouping, elementAt(1));
    });
    ...
    it('should group by key provided by the selector', () => {
      ...
      function attempt(): void {
        ...
        const COMPANY_A = 'Microsoft',
          COMPANY_B = 'Hell Corp.';
        const objArray = [
          { Name: 'John', Id: 0, Salary: 1300.0, Company: COMPANY_A },
          { Name: 'Peter', Id: 1, Salary: 4800.5, Company: COMPANY_A },
          { Name: 'Sandra', Id: 2, Salary: 999.99, Company: COMPANY_A },
          { Name: 'Me', Id: 3, Salary: 1000000000.0, Company: COMPANY_B },
        ];
