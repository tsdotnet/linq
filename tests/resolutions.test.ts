...
describe('resolutions/', () => {
	...
	describe('hasAny(sequence)', () => {
		it('should return true if not empty', () => {
			...
			expect(hasAny(b)).toBe(true);
		});

		it('should return false if empty', () => {
			expect(hasAny([])).toBe(false);
			expect(hasAny(linq([]))).toBe(false);
			expect(hasAny(emptyIterable)).toBe(false);
		...
		describe('single/', () => {
			...
			describe('single', () => {
				...
				it('should throw if empty', () => {
					...
					expect(() => single(linq([]))).toThrow();
				});
				it('should throw if more than one entry', () => {
					expect(() => single(c)).toThrow();
					expect(() => single(linq(c))).toThrow();
				});
			});
			...
			describe('singleOrDefault()', () => {
				...
				it('should return undefined if empty', () => {
					expect(singleOrDefault()([])).toBeUndefined();
					expect(singleOrDefault()(linq([]))).toBeUndefined();
				});
				it('should throw if more than one entry', () => {
					expect(() => singleOrDefault()(c)).toThrow();
					expect(() => singleOrDefault()(linq(c))).toThrow();
				});
			});
			...
			describe('singleOrDefault(value)', () => {
				...
				it('should throw if more than one entry', () => {
					expect(() => singleOrDefault(-1)(c)).toThrow();
					expect(() => singleOrDefault(-1)(linq(c))).toThrow();
				});
			});
		...
		describe('first/', () => {
			...
			describe('firstOrDefault()', () => {
				...
				it('should return undefined if empty', () => {
					...
					expect(firstOrDefault()(linq([]))).toBeUndefined();
				});
			});
		...
		describe('last/', () => {
			...
			describe('lastOrDefault()', () => {
				...
				it('should return undefined if empty', () => {
					...
					expect(lastOrDefault()(linq([]))).toBeUndefined();
				});
			});
		...
		describe('elementAt/', () => {
			describe('elementAt(index)', () => {
				...
				it('should throw if out of bounds', () => {
					expect(() => elementAt(-1)(a)).toThrow();
					expect(() => elementAt(a.length)(a)).toThrow();
					expect(() => elementAt(-1)(b)).toThrow();
					expect(() => elementAt(a.length)(b)).toThrow();
				});
			});
			...
			describe('elementAtOrDefault(index)', () => {
				...
				it('should throw if negative', () => {
					expect(() => elementAtOrDefault(-1)(a)).toThrow();
					expect(() => elementAtOrDefault(-1)(b)).toThrow();
				});
				it('should return undefined if out of bounds', () => {
					expect(elementAtOrDefault(a.length)(a)).toBeUndefined();
					expect(elementAtOrDefault(a.length)(b)).toBeUndefined();
				});
			});
			...
			describe('elementAtOrDefault(index, defaultValue)', () => {
				...
				it('should throw if negative', () => {
					expect(() => elementAtOrDefault(-1, -1)(a)).toThrow();
					expect(() => elementAtOrDefault(-1, -1)(b)).toThrow();
				});
				it('should return the defaultValue if out of bounds', () => {
					expect(elementAtOrDefault(a.length, -1)(a)).toBe(-1);
					expect(elementAtOrDefault(a.length, -1)(b)).toBe(-1);
				});
			});
	...
	describe('procedures/', () => {
		...
		describe('sum(sequence)', () => {
			...
			it('should return NaN if any are NaN', () => {
				expect(isNaN(sum([1, NaN, 2]))).toBe(true);
			});
		});
		...
		describe('average(sequence)', () => {
			...
			it('should return NaN if any are NaN', () => {
				expect(isNaN(average([1, NaN, 2]))).toBe(true);
			});
		});
		...
		describe('product(sequence)', () => {
			...
			it('should return NaN of no entries', () => {
				expect(isNaN(product([]))).toBe(true);
			});
			it('should return NaN if any are NaN', () => {
				expect(isNaN(product([1, NaN, 2]))).toBe(true);
			});
		});
	...
	describe('sequenceEqual(sequence)', () => {
		it('should return true if sequence matches', () => {
			...
			expect(sequenceEqual(sequence)(sequence)).toBe(true);
			expect(sequenceEqual(linq(sequence))(sequence)).toBe(true);
		});

		it('should return false if sequence does not match', () => {
			const sequence = [1, 2, 3, 4];
			expect(sequenceEqual(sequence)([1, 2, 3, 4, 5])).toBe(false);
			expect(sequenceEqual(sequence)([2, 3, 4, 5])).toBe(false);
		});
	});
	...
	describe('toMap(sequence, MappingMode.Throw)', () => {
		it('should not throw if no duplicates', () => {
			expect(() => toMap<TestItem, number>(e => e.b)(testItems[0]!.children!)).not.toThrow();
		...
		it('should throw for duplicates', () => {
			expect(() => toMap<TestItem, number>(e => e.a)(testItems)).toThrow(); // Duplicate keys.
		});
	});

	describe('toMap(sequence, MappingMode.Keep)', () => {
		it('should keep any existing values', () => {
			expect(() => toMap<TestItem, number>(e => e.b, MappingMode.Keep)(testItems)).not.toThrow();
	...
	describe('toMap(sequence, MappingMode.Overwrite)', () => {
		it('should overwrite existing values', () => {
			expect(() => toMap<TestItem, number>(e => e.b, MappingMode.Overwrite)(testItems)).not.toThrow();
